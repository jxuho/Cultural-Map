...
const FavoriteSites = () => {
  ...
  const renderContent = () => {
    ...
    if (isLoadingFavorites) {
      return (
        <div className="text-center grow flex items-center justify-center flex-col">
          ...
          <p className="text-gray-600 ml-2">Loading favorite sites...</p>
        </div>
      );
    }
    ...
    if (!sortedFavorites || sortedFavorites.length === 0) {
      // Check sortedFavorites for empty state
      return (
        ...
        {sortedFavorites.map(
          (
            ...
            >
              ...
              >
                <span className="text-lg font-semibold text-gray-800 grow pr-4 break-all">
                  {site.name}
                </span>
                <button
