...
export const fetchAllCulturalSites = async (
...
export const fetchCulturalSiteById = async (
  id: string,
): Promise<Place | null> => {
  ...
  try {
    const response = await axiosInstance.get<
      ApiResponse<{ culturalSite: Place }>
    >(`/cultural-sites/${id}`);
...
export const getNearbyOsm = async (
  lat: number,
  lon: number,
): Promise<Place[]> => {
...
export const createCulturalSite = async (
  siteData: Partial<Place>,
): Promise<Place> => {
  const response = await axiosInstance.post<
    ApiResponse<{ culturalSite: Place }>
  >('/cultural-sites', siteData);
  return response.data.data.culturalSite;
};

// update an existing cultural site (Admin)
export const updateCulturalSite = async (
  ...
  try {
    const response = await axiosInstance.put<
      ApiResponse<{ culturalSite: Place }>
    >(`/cultural-sites/${culturalSiteId}`, updateData);
    return response.data.data.culturalSite || null;
  } catch (error) {
    const err = error as AxiosError;
...
export const deleteCulturalSite = async (
  culturalSiteId: string,
): Promise<boolean> => {
