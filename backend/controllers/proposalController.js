...
const createProposal = asyncHandler(async (req, res, next) => {
  ...
  if (proposalType === 'update' || proposalType === 'delete') {
    ...
    if (!culturalSiteId) {
      return next(
        new AppError('Cultural site ID (culturalSite) is required.', 400),
      );
    }
    ...
    if (existingPendingProposal) {
      ...
      message +=
        ' for this cultural site has already been submitted and is pending review.';
  ...
  switch (proposalType) {
    case 'create':
      ...
      if (
        ...
        (sourceIdParts[0] !== 'node' &&
          sourceIdParts[0] !== 'way' &&
          sourceIdParts[0] !== 'relation') ||
        isNaN(parseInt(sourceIdParts[1]))
      ) {
        return next(
          new AppError(
      ...
      try {
        const osmResponse = await queryOverpass(
          singleElementQuery(osmType, osmId),
        );
        ...
        if (proposedSourceId !== actualSourceId) {
          console.warn(
            `Client provided sourceId mismatch: Expected ${actualSourceId}, Got ${proposedSourceId}`,
          );
          return next(
            new AppError(
        ...
        if (
          rawData.location.type !== 'Point' ||
          rawData.location.coordinates.length !== 2 ||
          !isValidLatLng(
        ...
        const osmCoord = [
          actualLocation.coordinates[0],
          actualLocation.coordinates[1],
        ];
        ...
        console.error(
          'Error fetching or processing OSM data during proposal creation:',
          error,
        );
        if (error.response) {
          console.error(
            'Overpass API Response Error:',
            error.response.status,
            error.response.data,
          );
          return next(
            new AppError(
              `OSM API error: ${error.response.status} - ${JSON.stringify(error.response.data)}`,
              error.response.status,
            ),
          );
        }
        return next(
          new AppError(
            `Unknown error fetching or processing OSM data: ${error.message}`,
            500,
          ),
        );
      }

      const proposedChangesForCreate =
        processOsmElementForCulturalSite(actualOsmElement);
      ...
      if (
        proposedChangesForCreate.category &&
        !CULTURAL_CATEGORY.includes(proposedChangesForCreate.category)
      ) {
        return next(
          new AppError(
            `Invalid category value: ${proposedChangesForCreate.category}`,
            400,
          ),
        );
      }
    ...
    case 'update':
      ...
      const culturalSiteToUpdate = await CulturalSite.findById(
        culturalSiteIdForUpdate,
      );
      ...
      for (const field of CULTURAL_SITE_UPDATABLE_FIELDS) {
        if (rawData[field] !== undefined) {
          if (
            JSON.stringify(culturalSiteToUpdate[field]) !==
            JSON.stringify(rawData[field])
          ) {
      ...
      if (
        proposedChangesForUpdate.category &&
        proposedChangesForUpdate.category.newValue &&
        !CULTURAL_CATEGORY.includes(proposedChangesForUpdate.category.newValue)
      ) {
        return next(
          new AppError(
            `Invalid category value: ${proposedChangesForUpdate.category.newValue}`,
            400,
          ),
        );
      }
    ...
    case 'delete':
      ...
      const culturalSiteToDelete = await CulturalSite.findById(
        culturalSiteIdForDelete,
      );
...
const getProposalsByUserId = asyncHandler(async (req, res, next) => {
  ...
  const proposals = await Proposal.find({ proposedBy: userId })
    .populate(
      'culturalSite',
      'name description category address website imageUrl openingHours',
    ) // Includes relevant cultural heritage information
...
const getAllProposals = asyncHandler(async (req, res, next) => {
  ...
  const proposals = await Proposal.find(filter)
    .populate(
      'culturalSite',
      'name description category address website imageUrl openingHours',
    ) // Populate some relevant cultural heritage information
...
const acceptProposal = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    if (proposal.status !== 'pending') {
      return next(
        new AppError('This proposal has already been processed.', 400),
      );
    }
    ...
    switch (proposal.proposalType) {
      case 'create':
        ...
        culturalSite = await CulturalSite.create([newCulturalSiteData], {
          session,
        });

        break;
      case 'update':
        culturalSite = await CulturalSite.findById(
        ...
        break;
      case 'delete':
        originalCulturalSite = await CulturalSite.findById(
          proposal.culturalSite,
        ).session(session);
        if (!originalCulturalSite) {
          console.warn(
            `Cultural site to delete ${proposal.culturalSite} not found. It might have been deleted already.`,
          );
        } else {
          // 1. Delete associated reviews (already correct based on previous understanding)
          await Review.deleteMany({
            culturalSite: originalCulturalSite._id,
          }).session(session);
          console.log(
            `Deleted reviews for cultural site: ${originalCulturalSite._id}`,
          );
          ...
          console.log(
            `Removed cultural site ${originalCulturalSite._id} from users' favorite lists.`,
          );
          ...
          console.log(
            `Cultural site ${originalCulturalSite._id} successfully deleted.`,
          );
        }
        ...
        if (originalCulturalSite && originalCulturalSite.sourceId) {
          ...
          console.log(
            `SourceId ${originalCulturalSite.sourceId} added to ExcludeSourceId collection.`,
          );
        } else {
          console.warn(
            `Could not add sourceId to ExcludeSourceId for deleted cultural site. proposal.culturalSite: ${proposal.culturalSite}`,
          );
        }
    ...
    return next(
      error instanceof AppError
        ? error
        : new AppError(`Error accepting proposal: ${error.message}`, 500),
    );
  }
});
